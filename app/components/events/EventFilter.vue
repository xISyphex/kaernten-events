...
const activeFilterPills = computed(() => {
  ...
  if (appliedTopOnly.value)
    pills.push({ label: 'TOP-Events', remove: () => { topOnly.value = false; syncAppliedFilters() } })

  appliedTowns.value.forEach(t =>
    pills.push({ label: t, remove: () => { toggleTown(t); stagedTowns.value = stagedTowns.value.filter(x => x !== t); syncAppliedFilters() } }))
  appliedGroups.value.forEach(g =>
    pills.push({ label: g, remove: () => { stagedGroups.value = stagedGroups.value.filter(x => x !== g); syncAppliedFilters() } }))
  appliedThemes.value.forEach(th =>
    pills.push({ label: th, remove: () => { stagedThemes.value = stagedThemes.value.filter(x => x !== th); syncAppliedFilters() } }))
    ...
    >
      ...
      <div class="flex gap-3">
        <button
          v-if="isApplied"
          @click="clearFilters"
          class="h-12 rounded-2xl flex flex-row gap-2 items-center border border-gray-300 bg-white px-5 text-sm font-medium hover:bg-gray-50 text-black transition"
        >
