...
const allFilteredEvents = computed(() => {
  ...
  return (source as Record<string, any>[]).filter((ev) => {
    ...
    if (filters.search) {
      ...
      if (!title.includes(q) && !description.includes(q) && !criteriaGroups.includes(q) && !themeGroups.includes(q)) return false
    }
    ...
    if (filters.themes.length > 0) {
      if (!filters.themes.some((themeFilter) => themeGroups.includes(String(themeFilter).toLowerCase()))) return false
    }
...
console.log('townOptions', townOptions.value)
const groupOptions = computed(() => filterOptions.value?.groups ?? [])
console.log('groupOptions', groupOptions.value)
const themeOptions = computed(() => filterOptions.value?.themes ?? [])
console.log('themeOptions', themeOptions.value)

function onFiltersUpdate(next: { search?: string; towns?: string[]; groups?: string[]; themes?: string[]; topOnly?: boolean; startDate?: string | null; endDate?: string | null }) {
  filters.search = next.search ?? ''
  filters.towns = next.towns ?? []
  filters.groups = next.groups ?? []
  filters.themes = next.themes ?? []
  ...
  <div class="min-h-screen bg-cover bg-center bg-no-repeat p-8" style="background-image: url('/images/bg.jpg')">
    <div class="max-w-6xl mx-auto space-y-6">
      <div class="rounded-2xl bg-white/60 p-6 shadow-sm border border-gray-200">
        ...
        <div class="mt-4">
          <EventFilter :events="events" :towns="townOptions" :groups="groupOptions" :themes="themeOptions" @update="onFiltersUpdate" />
        </div>
